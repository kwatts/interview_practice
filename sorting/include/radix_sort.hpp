...
struct RadixValueTraits
{
    ...
    inline static RadixKeyT getKey(ValueT const &v)
    {
        return v;
    }
};
...
inline std::size_t getBucket4Bit(RadixKeyT const &key, std::size_t bucket_shift)
{
...
void radixSort(IteratorT start, IteratorT end)
{
    ...
    static_assert(IsUnsignedIntegral<RadixKeyT>::value,
            "Radix key must be unsigned integer type! Specialize template for \"IsUnsignedIntegral\"");
