...
BOOST_AUTO_TEST_CASE(SortingPerformance)
{
    ...
    sorting_practice::testing::createRandomVector<UnsignedIntT>(elements, 10000, LARGE_VEC);

    UnsignedIntT* start = &elements.data()[0];
    UnsignedIntT* end   = start + elements.size();

    // radix
    BOOST_REQUIRE(sorting_practice::testing::testPerformance(start, end,
            RadixSortFunctorWrapper<UnsignedIntT*>(),
            "radix_sort"));

    // heap
    BOOST_REQUIRE(sorting_practice::testing::testPerformance(start, end,
            HeapSortFunctorWrapper<UnsignedIntT*>(),
            "heap_sort"));
}
...
struct RadixValueTraits<MyStruct>
{
    ...
    static RadixKeyT getKey(MyStruct const &v)
    {
        return v.key;
    }
};
...
BOOST_AUTO_TEST_CASE(RadixSortingCustomType)
{
    ...
    {
        MyStruct mys; mys.key = elements[i];
    ...
    // radix
    BOOST_REQUIRE(sorting_practice::testing::testPerformance(start, end,
            RadixSortFunctorWrapper<MyStruct*>(),
            "radix_sort"));
}
