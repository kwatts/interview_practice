...
void createRandomVector(std::vector<IntT> &vec, IntT max_value,
                        std::size_t max_size) {
...
double timeDiff(clock_t start, clock_t end) {
  return (static_cast<double>(end) - static_cast<double>(start)) /
...
template <typename IteratorT1, typename IteratorT2,
          typename LessT = sorting_practice::Less<
              typename IteratorTraits<IteratorT1>::ValueType> >
bool rangesEquivalent(IteratorT1 start, IteratorT1 end, IteratorT2 other,
                      LessT const &cmp = LessT()) {
  IteratorT1 it = start;
...
bool testPerformance(IteratorT start, IteratorT end, SortFunctionT sort_fn,
                     std::string const &sort_name) {
  // Make a copy of the input elements, allows both algorithms to have same
  // input
  typedef typename sorting_practice::IteratorTraits<IteratorT>::ValueType
      ValueT;
  ...
  std::cout << "Time for " + sort_name + ": "
            << (timeDiff(custom_start, custom_stop) * 1.0e3) << "ms"
            << std::endl;
  std::cout << "Time for std sort:  " << (timeDiff(std_start, std_stop) * 1.0e3)
            << "ms" << std::endl;
